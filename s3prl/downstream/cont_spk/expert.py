...
from .dataset import Contrastive_train
...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, downstream_expert, expdir, **kwargs):
        ...
        self.train_dataset = Contrastive_train(**train_config)
        ...
        self.objective = eval(self.modelrc['ObjectiveLoss'])(**objective_config) #Use NT_Xent loss
        # utils
    ...
    def wandb_init(self, train_config, ModelConfig):
        wandb.login()
        config = {}
        config.update(train_config)
        config.update(ModelConfig)
        wandb.init(project="Contrastive_Speaker", config=config)
    ...
    def get_dataloader(self, mode):
        ...
        #Only have training mode in contrastive learning
        if mode == 'train':
    ...
    def forward(self, mode, features, utter_idx, labels, records, **kwargs):
        ...
        if mode == 'train':
            feature_1, out_1, feature_2, out_2 = self.model(features_pad, attention_mask_pad.cuda())
            loss = self.objective(out_1, out_2)
